 ...
 */
async function searchKickChannels(query: string): Promise<ChannelSearchResult[] | null> {
  try {
    const slug = query.toLowerCase().trim();
    const response = await httpGet(`${API_CONFIG.kick.apiBaseUrl}/${encodeURIComponent(slug)}`);

    // 404 → channel doesn't exist; return empty array (not a failure)
...
/**
 * @brief Composable for channel name autocomplete search
 *
...
export function useChannelSearch(
  ...
  platform: MaybeRefOrGetter<Platform>
) {
  ...
  watchEffect(() => {
    ...
    // Clear stale results immediately when query/platform changes
    results.value = [];
    ...
    debounceTimer = setTimeout(async () => {
      ...
      const search = currentPlatform === "twitch" ? searchTwitchChannels : searchKickChannels;
