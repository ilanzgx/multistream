...
6. **Write Unit Tests:** Verify all parsing or data manipulation functions with `bun run desktop:test:backend` (NEVER run standard `cargo test` as it is slow and lacks process isolation) using explicit `// Arrange`, `// Act`, `// Assert` blocks.
