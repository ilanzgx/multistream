// ==UserScript==
// @name         Multistream Stream Filter & Engine
// @description  Player synchronization, un-pausing, and stream ad-filtering for Multistream
// @author       TwitchAdSolutions by pixeltris, with custom Multistream adaptations
// ==/UserScript==

if (window.location.hostname.includes("twitch.tv") || window.location.origin.includes("twitch.tv")) {
// ==UserScript==
...
(function() {
    ...
    // Isolate Twitch players so they don't pause each other
    try {
        // Redefine document properties to always be visible and focused
        try {
            ...
            const noopSetter = { set: function() {}, get: function() { return null; }, configurable: true };
            Object.defineProperty(window, 'onblur', noopSetter);
            Object.defineProperty(window, 'onfocus', noopSetter);
            Object.defineProperty(window, 'onstorage', noopSetter);
            Object.defineProperty(window, 'onpagehide', noopSetter);
            Object.defineProperty(document, 'onvisibilitychange', noopSetter);
            Object.defineProperty(document, 'onwebkitvisibilitychange', noopSetter);
        } catch (e) {}

        document.hasFocus = function() { return true; };
        window.SharedWorker = undefined;
        
        // Block BroadcastChannel completely
        ...
        // Block Web Locks API (used by modern sites for cross-tab sync)
        if (navigator.locks) {
            navigator.locks.request = function() { return new Promise(() => {}); };
        }
        ...
        // Hook IntersectionObserver so elements are always treated as fully visible/intersecting
                ...
                const observer = new OriginalIntersectionObserver((entries, obs) => {
                    const mockedEntries = entries.map(entry => {
                        return Object.create(entry, {
                            ...
                            rootBounds: { get: () => entry.boundingClientRect }
                        });
                    });
                ...
                const wrapped = function(e) {
                    try {
                        ...
                        if (typeof e.data === 'string') {
                            if (e.data.toLowerCase().includes('pause')) {
                                shouldBlock = true;
                            }
                        } else if (e.data && typeof e.data === 'object') {
                            const keysToCheck = ['type', 'event', 'method', 'command'];
                            for (let k = 0; k < keysToCheck.length; k++) {
                                const key = keysToCheck[k];
                                if (typeof e.data[key] === 'string' && e.data[key].toLowerCase().includes('pause')) {
                                    shouldBlock = true;
                                    break;
                                }
                            }
        ...
        const originalDocAEL = document.addEventListener;
        document.addEventListener = function(type, listener, options) {
            if (blockEventTypes.has(type)) {
                return;
            }
            return originalDocAEL.call(this, type, listener, options);
        };
    } catch(e) {}

    // Aggressive Auto-Resume for stuck players
    setInterval(() => {
        try {
            const videos = document.getElementsByTagName('video');
            if (videos.length > 0 && videos[0].paused && !videos[0].ended) {
                // Only force play if we have buffered data, to avoid spamming play on dead streams
    ...
    function declareOptions(scope) {
        ...
        scope.PlayerBufferingFix = false; // Diagnostic: disable buffering fix to see if hover pause still happens
        scope.PlayerBufferingDelay = 600;
        scope.PlayerBufferingSameStateCount = 6;
        scope.PlayerBufferingDangerZone = 1;
        scope.PlayerBufferingDoPlayerReload = false; // Back to pause/play to avoid autoplay policy
        scope.PlayerBufferingMinRepeatDelay = 20000;
...
return realFetch('data:video/mp4;base64,', options).then(function(response) {
                                resolve(response);
                            })['catch'](function(err) {
                                reject(err);
                            });
                                            ...
                                            for (let i = 0; i < lines.length - 1; i++) {
                                                if (lines[i].startsWith('#EXT-X-STREAM-INF')) {
                                                    ...
                                                    if (resSettings[codecsKey].startsWith('hev') || resSettings[codecsKey].startsWith('hvc')) {
                                                        ...
                                                        const newResolutionInfo = nonHevcResolutionList.toSorted((a, b) => {
...
if (streamInfo.ActiveBackupPlayerType != backupPlayerType) {
    ...
    function monitorPlayerBuffering() {
        ...
        // If the interval was throttled by the browser (background tab sleeping), reset the counter
        if (timeDiff > PlayerBufferingDelay * 2) {
                ...
                if (!player.core) {
                    ...
                    if (position !== undefined && bufferedPosition !== undefined) {
                        if (playerBufferState.hasStreamStarted &&
                            ...
                            timeDiff <= PlayerBufferingDelay * 2 // Only increment if we are not throttling
                        ) {
    ...
    const isAdblockDisabled = window.location.hash.includes("ms-adblock=0") || window.name.includes("ms-adblock=vanilla");
...
(function() {
    if (window.location.href.includes('twitch.tv') && window.location.search.includes('muted=true')) {
        let checkInterval = setInterval(() => {
            if (document.querySelector('video')) {
                clearInterval(checkInterval);
                let newUrl = new URL(window.location.href);
                newUrl.searchParams.set('muted', 'false');
...
if (window.location.hostname.includes("youtube.com") || window.location.origin.includes("youtube.com")) {

(function () {
  try {
    const isAdblockDisabled =
      window.location.hash.includes("ms-adblock=0") ||
      window.name.includes("ms-adblock=vanilla");

    if (isAdblockDisabled) return;
